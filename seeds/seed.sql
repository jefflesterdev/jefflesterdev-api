...
INSERT INTO experience_items (experience_id, type, title, content, pinned, is_pinned, pin_order, display_order) VALUES
    ...
    (1, 'responsibility', NULL,                                       'Acted as technical reference point for the development team and management, providing architectural input, code review, and knowledge transfer across projects.',                                                                                                    0, 0, NULL, 8),
    ...
    (1, 'achievement',    'SOAP Sync Middleware & Conflict Resolution', 'Engineered a middleware synchronisation layer to bridge the React Native driver app with the legacy SOAP backend. With no ability to modify the SOAP service, derived deterministic unique IDs from response data and tracked change and deletion state in a separate MSSQL table, enriching API responses with the metadata needed for reliable offline sync. Delivered a driver-facing conflict chain resolution UI allowing review of full change histories, selection of correct versions, or manual override.',    1, 1, 6, 10);
